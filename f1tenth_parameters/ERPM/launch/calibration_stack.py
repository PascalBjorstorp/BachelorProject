...
def _hardware(path: Path) -> dict:
    ...
    required = ['lidar_ip_address','base_frame_id','laser_frame_id','imu_frame_id',
                ...
                'imu_to_base_x_m','imu_to_base_y_m','imu_to_base_yaw_rad']
    missing=[x for x in required if x not in hw]
    if missing: raise ValueError(f'missing hardware fields: {missing}')
    return hw


def generate_description(hw: dict, *, candidate_patch: Path | None = None,
                         candidate_mode: str | None = None,
                         simulation: bool = False) -> LaunchDescription:
    ...
    if simulation:
      ...
      sensor=Node(package='f1tenth_gym_ros', executable='gym_bridge', name='bridge',
        parameters=[os.path.join(str(sim_share),'config','sim.yaml'),{
          ...
          'scan_distance_to_base_link':float(hw['laser_to_base_x_m']),
      ...
      sensor=Node(package='f1tenth_lidar', executable='hokuyo_scip_driver_node', name='hokuyo_scip_driver', parameters=[lidar_config,{'ip_address':str(hw['lidar_ip_address']),'skip':0,'cluster':1}], output='screen')
    transforms=[
      Node(package='tf2_ros', executable='static_transform_publisher', name='static_baselink_to_imu', arguments=['--x',str(float(hw['imu_to_base_x_m'])),'--y',str(float(hw['imu_to_base_y_m'])),'--z',str(float(hw.get('imu_to_base_z_m',0.0))),'--roll','0.0','--pitch','0.0','--yaw',str(float(hw['imu_to_base_yaw_rad'])),'--frame-id',str(hw['base_frame_id']),'--child-frame-id',str(hw['imu_frame_id'])], output='screen'),
    ]
    if not simulation:
    ...
    return LaunchDescription([
      ...
      *extra,
      ...
      # Isolated calibration capture: retain all 1080 samples. The normal
      # racing/AMCL launch remains on its reduced scan.
      sensor,
      *transforms,
    ])
...
def main() -> int:
    p=argparse.ArgumentParser(); p.add_argument('--config',type=Path,required=True); p.add_argument('--candidate-patch',type=Path); p.add_argument('--candidate-mode',choices=['velocity','accel']); p.add_argument('--simulation',action='store_true',help='Use the purpose-built open-room simulator; never starts hardware drivers.')
    a=p.parse_args()
    ...
    service=LaunchService(); service.include_launch_description(generate_description(_hardware(a.config.resolve()),candidate_patch=a.candidate_patch.resolve() if a.candidate_patch else None,candidate_mode=a.candidate_mode,simulation=a.simulation)); return service.run()
