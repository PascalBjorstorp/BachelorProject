...
def main()->int:
    ...
    accepted=bool(vel_cov_ok and bool(acov.coverage_ok.all()) and vel_ok and dyn_ok and acc_ok and odom_accel_ok and cross_ok)
    report={'candidate_velocity_command':vel_cmd,'candidate_velocity_odom':vel_odom,'candidate_velocity_coverage_ok':vel_cov_ok,'candidate_dynamic_odom':dyn_odom,'candidate_dynamic_high_drive':dyn_drive,'candidate_dynamic_high_brake':dyn_brake,'candidate_accel_command':accel_command,'candidate_accel_odom_derivative':accel_odom,'candidate_acceleration_coverage_ok':bool(len(acov)) and bool(acov.coverage_ok.all()),'candidate_cross_axis_odom':cross_metric,'candidate_cross_axis_coverage_ok':cross_cov_ok,'velocity_gates_ok':vel_ok,'dynamic_odom_gates_ok':dyn_ok,'accel_interface_gates_ok':acc_ok,'candidate_odom_acceleration_gate_ok':odom_accel_ok,'cross_axis_gate_ok':cross_ok,'accepted_for_permanent_review':accepted,'note':'Candidate shadow odometry is compared directly with LiDAR velocity during settled, high-demand and non-zero-steering candidate hold-outs. Coverage, command tracking, RMSE, signed bias and high-drive/high-brake error all gate acceptance.'}
