...
\end{equation}
where $v_\text{wheel}$ comes from ERPM and $v_\text{body}$ from IMU
integration. In the linear region ($|\kappa|<0.15$):
\begin{equation}\label{eq:Fx}
  F_x = C_x\,\kappa, \qquad F_x = m\,a_x.
\end{equation}

\paragraph{Procedure.}
Accelerate from standstill to \SI{3}{\metre\per\second}, then brake to stop.
Both $v_\text{wheel}$ and $v_\text{body}$ are recorded simultaneously.
A \SI{2}{\second} stationary bias calibration precedes the test.
...
\includefigure[width=0.48\textwidth]{long_Fx_vs_kappa.pdf}{%
  $F_x$ vs.\ $\kappa$ scatter with linear fit through the origin
  in the low-slip region.\label{fig:long-Fx}}

\noindent CSV columns used: \texttt{odom\_vx} ($v_\text{wheel}$),
\texttt{v\_imu} ($v_\text{body}$), \texttt{imu\_ax},
\texttt{slip\_ratio}, \texttt{phase}.
Use bias-corrected $a_x$ for both force estimation and IMU velocity integration.

\paragraph{Uncertainty.}
\begin{itemize}
  \item IMU integration drift accumulates; test kept short ($<\SI{10}{\second}$).
  \item 4WD distributes torque through differentials; $C_x$ is an
        aggregate combined value.
  \item Rolling resistance and aerodynamic drag contribute to $F_x=m\,a_x$
        (small at low speed).
\end{itemize}
