...
changes $+\delta \to -\delta$ and measuring the 10\%--90\% rise time
of the IMU yaw rate response:
\begin{equation}\label{eq:steer-rate}
  \dot{\delta}_\text{max} = \frac{2\,\delta_\text{cmd}}
                                 {\Delta t_{10\text{--}90}}.
\end{equation}
...
transitions, with 2-second pauses between steps.
...
\includefigure{steering_step_response.pdf}{%
  ...
  Lines are broken at the 2-second recording gaps between
...
\begin{tabular}{cS[table-format=2.1]S[table-format=3.0]S[table-format=2.2]}
\toprule
{Step} & {$\Delta\omega$ (deg/s)} & {Rise time (ms)} & {Rate (rad/s)} \\
\midrule
0 &  84.1 &  95 & 15.48 \\
1 & -79.2 &  78 & 17.66 \\
2 &  77.9 &  82 & 16.63 \\
...
5 & -75.9 &  96 & 13.84 \\
\midrule
\multicolumn{2}{c}{Average} & 87 & {$6.94^{\dagger}$} \\
\bottomrule
\end{tabular}

\smallskip\noindent${}^{\dagger}$Estimated as
$\dot{\delta}_\text{max} = 2 \times 0.3\;\text{rad}\;/\;
\SI{87}{\milli\second} = \SI{6.94}{\radian\per\second}$.
\end{table}
...
$\SI{6.94}{\radian\per\second}$ is substantially faster than the
simulation default of $\SI{3.2}{\radian\per\second}$.
...
$0.86 \times 6.94 \approx \SI{5.97}{\radian\per\second}$.
...
\begin{itemize}
  ...
  \item \textbf{Steering rate}: $\dot{\delta}_\text{max} =
        \SI{6.94}{\radian\per\second}$ vs.\ \SI{3.2}{\radian\per\second}.
        The real servo is faster than assumed; this relaxes the
        steering rate constraint in the MPC.
