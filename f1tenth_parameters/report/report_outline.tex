...
\includefigure[width=0.65\textwidth]{cornering_understeer_gradient.pdf}{%
  ...
  linear fit gives
  $K_\text{us} = \SI{0.0093}{\radian\per(\metre\per\second\squared)}$
...
$K_\text{us} = 0.0093 \pm 0.001\;\text{rad/(m/s}^2\text{)}$.
...
are $C_{Sf} = C_{\alpha f}/(m\,g) = 1.58$ and
$C_{Sr} = C_{\alpha r}/(m\,g) = 1.33$.
The absolute values depend on the assumed $\omega_n$; the ratio
$C_{\alpha f}/C_{\alpha r} \approx 1.19$ is less sensitive and
...
exceed the commanded values by up to $6.5\%$ at small angles.
...
$K_\text{us} = 0.0093\;\text{rad/(m/s}^2\text{)}$ and yields the
...
Understeer gradient   & $K_\text{us}$
  & 0.0093  & {--}   & \si{\radian\per(\metre\per\second\squared)}
  & Tested \\
...
$\omega_n$.  The ratio $C_{\alpha f}/C_{\alpha r} = 1.19$ is
better constrained.\\
...
\begin{itemize}
  ...
  \item \textbf{Understeer}: the vehicle understeers with
        $K_\text{us} = 0.012$, requiring progressively more steering
