...
\includefigure[width=0.65\textwidth]{cornering_understeer_gradient.pdf}{%
  ...
  linear fit gives
  $K_\text{us} = \SI{0.012}{\radian\per(\metre\per\second\squared)}$
  ($R^2 = 0.94$, $n = 20$ operating points, 5 runs).\label{fig:cornering-Kus}}

The steering angle used in the regression is the calibrated wheel
angle from the VESC servo mapping
(Section~\ref{sec:vesc-cal}).  The measured understeer gradient is
$K_\text{us} = 0.012 \pm 0.001\;\text{rad/(m/s}^2\text{)}$.
...
\includefigure[width=0.7\textwidth]{cornering_solution_range.pdf}{%
  ...
  the measured $K_\text{us} = 0.012$ as a function of the assumed
...
are $C_{Sf} = C_{\alpha f}/(m\,g) = 1.71$ and
$C_{Sr} = C_{\alpha r}/(m\,g) = 1.30$.
The absolute values depend on the assumed $\omega_n$; the ratio
$C_{\alpha f}/C_{\alpha r} \approx 1.31$ is less sensitive and
...
Understeer gradient   & $K_\text{us}$
  & 0.012  & {--}   & \si{\radian\per(\metre\per\second\squared)}
  & Tested \\
...
(Section~\ref{sec:cornering}); absolute values depend on assumed
$\omega_n$.  The ratio $C_{\alpha f}/C_{\alpha r} = 1.31$ is
better constrained.\\
