...
static void test_vehicle_control_saturation(void)
{
    ...
    normal.acceleration_meters_per_second_squared = FP_CONST(5.0);
    ...
    check_fp("Normal force unchanged", sat.acceleration_meters_per_second_squared, 5.0, TOLERANCE_TIGHT);
    
    /* Over-limit steering */
    ControlInput_t over_steer;
    over_steer.steering_angle_radians = FP_CONST(1.0);  /* Way over limit */
    over_steer.acceleration_meters_per_second_squared = FP_CONST(5.0);
    ...
    over_force.acceleration_meters_per_second_squared = FP_CONST(100.0);  /* Way over limit */
    
    sat = vehicle_model_saturate_control(&over_force);
    check_fp("Over-limit force clamped", sat.acceleration_meters_per_second_squared, max_force, TOLERANCE_TIGHT);
    
    /* Negative force (should clamp to min, which allows braking) */
    ControlInput_t neg_force;
    neg_force.steering_angle_radians = 0;
    neg_force.acceleration_meters_per_second_squared = FP_CONST(-100.0);
    
    sat = vehicle_model_saturate_control(&neg_force);
    check_fp("Negative force clamped to min", sat.acceleration_meters_per_second_squared, min_force, TOLERANCE_TIGHT);
}
...
static void stress_qp_step_sizes(void)
{
    ...
    for (int si = 0; si < n_steps; si++) {
        ...
        char name[64];
        snprintf(name, sizeof(name), "Step %.3f: converges", step_sizes[si]);
        check_condition(name,
                        status == QP_STATUS_OPTIMAL);
    }
}
    ...
    for (int hi = 0; hi < 9; hi++) {
        for (int vi = 0; vi < 5; vi++) {
            for (int si = 0; si < 6; si++) {
                ...
                /* Dynamic model checks for 6-state with acceleration input:
                 * A[3][3]: dFx/dvx=0, so mainly 1 + dt*(-dFyf_dvx*sin(d))/m
                 *   Should be close to 1.0 at small steering angles.
                 * B[3][1] = dt = 0.05 (direct accel→velocity)
