...
BASE_OVERRIDES = {
    ...
    # Vehicle and actuation parameters are fixed constants in the simulator.
    "Q_LAT":  1000.0,
    "Q_HDG": 28.8,
    "Q_VEL": 30.0,
    "Q_LAT_VEL": 1.04,
...
def parse_csv_result(stdout: str, return_code: int) -> dict:
    """Parse the machine-readable summary emitted by test_fpga_csim_drive."""
...
def run_single_scenario(params: dict, binary: str, scenario: dict, seed: int) -> dict:
    ...
    # Align test_fpga_csim_drive's internal wall-margin floor (half-width + BODY_SAFETY_MARGIN)
...
def main():
    ...
    # test_fpga_csim_drive floors effective margin to (vehicle_half_width + body_safety_margin).
    ...
    # Build binary
    binary_name = f"test_fpga_csim_drive_{os.getpid()}_{int(time.time())}"
    if os.name == "nt":
        binary_name += ".exe"
    ...
    ret = subprocess.run([
        ...
        set -e
        cd {PROJECT_DIR}
        g++ -D_GNU_SOURCE -O2 -Wall -Wno-unknown-pragmas -I include \
            -c testbench/test_fpga_csim_drive.c -o /tmp/test_fpga_csim_drive.o
        gcc -D_GNU_SOURCE -O2 -Wall -Wno-unknown-pragmas -I include \
            -c src/fp_math_hls.c -o /tmp/fp_math_hls.o
        gcc -D_GNU_SOURCE -O2 -Wall -Wno-unknown-pragmas -I include \
            -c src/vehicle_model_hls.c -o /tmp/vehicle_model_hls.o
        gcc -D_GNU_SOURCE -O2 -Wall -Wno-unknown-pragmas -I include \
            -c src/riccati_solver_hls.c -o /tmp/riccati_solver_hls.o
        gcc -D_GNU_SOURCE -O2 -Wall -Wno-unknown-pragmas -I include \
            -c src/mpc_riccati_hls.c -o /tmp/mpc_riccati_hls.o
        g++ -D_GNU_SOURCE -O2 -Wall -Wno-unknown-pragmas -I include \
            -x c++ -c src/mpc_fpga_top.cpp -o /tmp/mpc_fpga_top.o
        g++ -o {binary_name} /tmp/test_fpga_csim_drive.o /tmp/fp_math_hls.o \
            /tmp/vehicle_model_hls.o /tmp/riccati_solver_hls.o /tmp/mpc_riccati_hls.o \
            /tmp/mpc_fpga_top.o -lm
        """
