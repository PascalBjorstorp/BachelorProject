...
# atan LUT: map [0,1] into 1024 segments of atan
atan_vals = [math.atan(i / N) for i in range(N + 1)]

# recip LUT for range [0.5,1) with 10-bit index (1024 entries)
FP_FRAC_BITS = 18
...
for v in atan_vals:
    header.append('    FP_QP_CONST(%s),' % fmt(v))
header.append('};\n')
header.append('static const int32_t recip_lut[1024] = {')
for v in recip:
    header.append('    %d,' % v)
header.append('};\n')
header.append('#endif')
