...
FrenetState_t mpc_predict_frenet_next_state(
    ...
    MpcSolverResult_t *result)
{
    ...
    /* Steering actuator lag (δ_eff = first-order lag of δ_actual). alpha = dt/tau,
     * clamped to [0,1] so the discrete pole (1-alpha) stays non-negative/stable. */
    float steer_lag_tau = get_env_float("MPC_STEER_LAG_TAU", STEER_LAG_TAU_S);
    if (!isfinite(steer_lag_tau) || steer_lag_tau < config.time_step)
        steer_lag_tau = config.time_step;
    const float steer_lag_alpha = config.time_step / steer_lag_tau;

    /* Option A (optional, off by default): host-side dead-time compensation.
     * Advance the initial Frenet tracking state by the steering dead-time so the
     * MPC plans from where the car will be when its command lands. Pair with a
     * lag-only MPC_STEER_LAG_TAU=0.034 so the dead-time is not double-counted. */
    FrenetState_t frenet_dc;
    float steer_delay = get_env_float("MPC_STEER_DELAY_S", STEER_DELAY_COMP_S);
    if (isfinite(steer_delay) && steer_delay > 0.0f) {
        frenet_dc = *frenet;
        const float kappa0 = reference_trajectory[0].path_curvature;
        const float vxc = frenet_dc.flong_vel;
        frenet_dc.fhead_error += (frenet_dc.fyaw_rate - vxc * kappa0) * steer_delay;
        frenet_dc.flat_error  += (frenet_dc.flat_vel + vxc * frenet_dc.fhead_error) * steer_delay;
        frenet = &frenet_dc;
    }
    ...
    for (int k = 0; k < N; k++) {
        ...
        sd->B[IDX_DELTA_EFF][0] = 0;     /* δ_eff has no direct control input (lag of δ_actual) */
        sd->B[IDX_DELTA_EFF][1] = 0;
        ...
        /* Steering effect on the Frenet states enters through δ_eff (what the
         * tyres see), not the commanded δ_actual — this is the actuator lag. */
        for (int i = 0; i < 5; i++)
            sd->A[i][IDX_DELTA_EFF] = B_step[i][0];
        ...
        /* δ_actual integrator (δ_actual_{k+1} = δ_actual_k + dt*δ̇) */
        sd->A[IDX_DELTA_ACTUAL][IDX_DELTA_ACTUAL] = 1.0f;

        /* δ_eff actuator lag: δ_eff_{k+1} = (1-α)·δ_eff_k + α·δ_actual_k. */
        sd->A[IDX_DELTA_EFF][IDX_DELTA_EFF]    = 1.0f - steer_lag_alpha;
        sd->A[IDX_DELTA_EFF][IDX_DELTA_ACTUAL] = steer_lag_alpha;

        /* Prev-control rows/cols: already zeroed above */
        ...
        sd->Q_diag[IDX_DELTA_EFF] = 0.0f;  /* No direct weight; δ_eff is penalised via its coupling into ey/epsi. */
        ...
        sd->q[IDX_DELTA_EFF] = 0;   /* No tracking ref for δ_eff (zero Q) */
        ...
        /* δ_actual and δ_eff: physical steering angle limit */
        sd->x_lb[IDX_DELTA_ACTUAL] = -VP_MAX_STEERING_RAD;
        sd->x_ub[IDX_DELTA_ACTUAL] = VP_MAX_STEERING_RAD;
        sd->x_lb[IDX_DELTA_EFF] = -VP_MAX_STEERING_RAD;
        sd->x_ub[IDX_DELTA_EFF] = VP_MAX_STEERING_RAD;

        /* Prev controls: unconstrained */
    ...
    if (N > 0) {
        ...
        terminal_x_lb[IDX_DELTA_EFF] = -VP_MAX_STEERING_RAD;
        terminal_x_ub[IDX_DELTA_EFF] = VP_MAX_STEERING_RAD;
    ...
    x0[IDX_DELTA_ACTUAL] = actual_steering_angle;      /* Commanded steering (servo echo) */
    x0[IDX_DELTA_EFF] = actual_steering_angle;         /* Effective (tyre) steering; not measured, init = commanded */
     ...
     * (mpc_full_hls.cpp Step 5): cold start on first call, model-signature
