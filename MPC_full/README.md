# mpc_full — CPU MPC for the F1TENTH Stack

> **Fork of `mpc_riccati` (`../MPC/`).** Created 2026-06-26 as the development
> variant for steering actuator-dynamics modelling (steering dead-time ~0.083 s +
> first-order lag τ~0.034 s): host-side delay compensation (Option A) plus an
> augmented `delta_eff` lag state (Option B). Package renamed `mpc_riccati`→`mpc_full`,
> ROS nodes `mpc_hardware_node`→`mpc_full_hardware_node`, `mpc_node`→`mpc_full_node`.
> It publishes the same topics as the original, so run one or the other (or remap).
...
```bash
colcon build --packages-select mpc_full
...
```bash
ros2 launch mpc_full mpc_launch.py \
