...
#define NX_AUG 9                                         /* Augmented state size: Frenet(5) + delta_actual + delta_eff + prev controls(2). */
#define IDX_EY 0                                         /* Position of lateral error (ey) in the augmented vector. */
#define IDX_DELTA_ACTUAL 5                               /* Commanded steering state (integral of commanded delta-rate). */
#define IDX_DELTA_EFF 6                                  /* Effective steering the tyres see; first-order lag of delta_actual (actuator dynamics). */
...
#define NX_DENSE 7                                       /* Dense A-block width (Frenet + delta_actual + delta_eff) before sparse prev-control tail. */
...
/* Steering actuator dynamics (mpc_full): the tyres see delta_eff, a first-order
 * lag of the commanded delta_actual, modelled across the whole horizon (Option B).
 * Effective steering FOPDT from the 2026-06-25 calibration: dead-time ~0.083 s,
 * lag tau ~0.034 s. Defaults lump both into one pole (tau_eff = L + tau = 0.117 s)
 * so B works alone. If host-side dead-time compensation (Option A) is enabled, set
 * MPC_STEER_LAG_TAU=0.034 and MPC_STEER_DELAY_S=0.083 so they don't double-count. */
#define STEER_LAG_TAU_S 0.117f                           /* delta_eff first-order lag time constant (s); env MPC_STEER_LAG_TAU. */
#define STEER_DELAY_COMP_S 0.0f                          /* Host-side dead-time compensation (s); 0 = off (Option A); env MPC_STEER_DELAY_S. */
...
#define MPC_MODEL_SIGNATURE 2                            /* Bumped to 2: added delta_eff actuator-lag state (mpc_full). Forces cold start. */
