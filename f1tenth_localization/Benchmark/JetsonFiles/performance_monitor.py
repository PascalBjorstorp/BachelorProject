...
class PerformanceMonitor(Node):
    def __init__(self):
        ...
        self.declare_parameter('sample_rate_hz', 5.0)  # Higher rate to catch spikes
        ...
        # CSV headers - config info is encoded in the filename
        headers = [
            ...
            # AMCL covariance (convergence quality)
            'covariance_xx', 'covariance_yy', 'covariance_yaw_yaw',
        ]
        ...
        self.latest_covariance = (0.0, 0.0, 0.0)  # (xx, yy, yaw_yaw) from AMCL
        
        # Frame alignment: offset between map frame (AMCL) and odom frame (ground truth)
        # Recomputed after AMCL has had time to converge (after N pose updates)
        self.frame_alignment_offset = None  # (dx, dy, dyaw) to align odom frame to map frame
        self.alignment_pose_count = 0  # Count pose updates before computing alignment
        self.ALIGNMENT_WARMUP = 10  # Wait for this many unique AMCL updates before aligning
        ...
        self.ground_truth_sub = self.create_subscription(
            Odometry, odom_topic, self.ground_truth_callback, 50
        )
    ...
    def _try_compute_alignment(self, amcl_pose):
        """Compute frame alignment using the given AMCL pose and closest ground truth pose.
        Called after AMCL has warmed up (ALIGNMENT_WARMUP pose updates) to avoid
        aligning during initial convergence when estimates are poor."""
        import math
        if not self.ground_truth_history:
            return
        
        # Use the latest ground truth pose (closest in time to the current AMCL pose)
        ...
        self.get_logger().info(
            f'Frame alignment computed (after {self.alignment_pose_count} AMCL updates): '
            f'dx={dx:.3f}m, dy={dy:.3f}m, dyaw={math.degrees(dyaw):.1f}°'
        )
    ...
    def pose_callback(self, msg: PoseWithCovarianceStamped):
        ...
        # Compute frame alignment after AMCL has warmed up (converged)
        # Count unique scan updates (not repeated cached poses)
        # Alignment is computed once after ALIGNMENT_WARMUP unique updates
        # This line runs before dedup check below since we need the pose for alignment
        ...
        # Extract covariance diagonal: xx, yy, yaw-yaw from 6x6 row-major matrix
        cov = msg.pose.covariance
        self.latest_covariance = (cov[0], cov[7], cov[35])
        
        # Count unique AMCL updates for alignment warmup
        self.alignment_pose_count += 1
        if self.frame_alignment_offset is None and self.alignment_pose_count >= self.ALIGNMENT_WARMUP:
            self._try_compute_alignment(amcl_pose)
    ...
    def sample_callback(self):
        ...
        if self.psutil:
            ...
            if len(self.amcl_cpu_samples) > 20:  # ~few seconds window
        ...
        # Prepare data row (config info is in the filename, not repeated per row)
        data = {
            ...
            'covariance_xx': self.latest_covariance[0],
            'covariance_yy': self.latest_covariance[1],
            'covariance_yaw_yaw': self.latest_covariance[2],
        }
