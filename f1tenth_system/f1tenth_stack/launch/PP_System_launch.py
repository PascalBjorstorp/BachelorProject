#!/usr/bin/env python3
"""
Launch file that starts the full System launch immediately,
then starts Pure Pursuit after a fixed delay.

Behavior:
  1. Include System_launch.py from f1tenth_stack.
  2. After 5 seconds, start a Pure Pursuit container using
     the same arguments and node settings as pure_pursuit_launch.py.
"""
...
def generate_launch_description():
    ...
    # Pure Pursuit launch arguments (copied from pure_pursuit_launch.py).
    ...
    # System launch include.
    system_include = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(system_launch_path)
    )

    # Pure Pursuit container (copied from pure_pursuit_launch.py).
    ...
    delayed_pure_pursuit = TimerAction(
        period=5.0,
        actions=[
            LogInfo(msg='Starting Pure Pursuit after 5.0 seconds...'),
