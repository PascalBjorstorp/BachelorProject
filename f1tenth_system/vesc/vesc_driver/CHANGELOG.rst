...
1.2.1 (2024-XX-XX)
------------------
* Added imu_frame parameter for TF frame_id configuration
* Added configurable IMU covariance parameters:
  - imu_orientation_covariance (default: 0.0001)
  - imu_angular_velocity_covariance (default: 0.00002)
  - imu_linear_acceleration_covariance (default: 0.0004)
* Increased timer rate from 50Hz to 200Hz for better IMU sampling
* Fixed IMU unit conversions:
  - Acceleration: g's → m/s² (multiply by 9.80665)
  - Angular velocity: deg/s → rad/s (multiply by π/180)
* Updated default config file (vesc_config.yaml) with new parameters
* Fixed vesc_device_lookup script shebang and distro-agnostic LD_LIBRARY_PATH
* Fixed unsigned shift undefined behavior in vesc_packet.cpp: (1 << 31) → (1u << 31)
* Contributors: VAUL, Humble
