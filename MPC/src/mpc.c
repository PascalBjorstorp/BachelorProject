...
static MpcConfiguration_t get_default_configuration(void)
{
     ...
     */
    config.weight_lateral_error  = FP_CONST(10.0);                   /* 3.0 - keep car on path (was 2.0) */
    config.weight_heading_error  = FP_CONST(7.0);                   /* 5.0 - heading alignment (restored from 3.0 — needed for corner tracking) */
    config.weight_velocity   = FP_CONST(0.05);                    /* 0.05 - deprioritize speed vs heading */
    config.weight_lateral_velocity = FP_CONST(0.3);               /* 0.3 - penalize sideslip to prevent drifting */
    config.weight_yaw_rate   = FP_CONST(0.07);                    /* 0.05 - light yaw damping for corner exit; 0 causes corner exit instability; >0.15 causes curve entry aggression */
     ...
     * a safety net. */
    config.weight_steering_rate  = FP_CONST(1.0);                    /* 0.5 - smooth steering; penalizes step-to-step steering changes */
    ...
    fixed_point_t *linear_cost_vector)
{
    ...
    /* ---------------------------------------------------------------
     * Conservative B scaling at high speed — DISABLED.
     *
     * Previously inflated B[3][0] and B[4][0] by vx/8 at high speed,
     * which made the MPC command less steering (thought steering was
     * more effective). This was a hack to compensate for model mismatch.
     *
     * Now that mu, C_Sf, C_Sr, and the Pacejka model correctly match
     * the simulator, the B-matrix entries are accurate and this
     * scaling causes understeer (MPC commands too little steering).
     * The Pacejka tire model already reduces the effective stiffness
     * at high slip angles, providing natural saturation.
     * --------------------------------------------------------------- */
    ...
    /* Phi[m] = A[m] * Phi[m-1], with A[1][2] varying per step */
    for (int m = 1; m < horizon_steps; m++)
    {
        /* Update A[1][2] for this prediction step's curvature */
        A[1][2] = fp_neg(fp_mul(current_configuration.time_step_seconds,
                                reference_trajectory[m].path_curvature_radians_per_meter));
     ...
     *
     * In Frenet frame: state = [e_y, e_psi, v_x, v_y, omega, omega_w]
    ...
    {
        /* Update A[1][2] for this step's curvature (same as Phi computation) */
        A[1][2] = fp_neg(fp_mul(current_configuration.time_step_seconds,
         ...
         * Frenet state order: [e_y, e_psi, v_x, v_y, omega, omega_w] */
        d[k][0] = fp_sub(x_free[0],
            reference_trajectory[k].reference_lateral_error_meters);
        d[k][1] = normalize_angle(fp_sub(x_free[1],
            reference_trajectory[k].reference_heading_error_radians));
        d[k][2] = fp_sub(x_free[2],
        ...
        /* Save free-response e_y for wall constraint building */
        saved_free_response_ey[k] = x_free[0];

        /* Advance for next iteration, normalizing heading error to [-pi, pi] */
        for (int s = 0; s < STATE_DIMENSION; s++)
        {
            x_prev[s] = x_free[s];
        }
        /* Normalize predicted heading error to prevent wrap-around issues */
    ...
    MpcSolverResult_t *result)
{
    ...
    /* Set problem dimensions (controls + slacks) */
