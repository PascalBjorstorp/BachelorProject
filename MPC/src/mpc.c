...
static MpcConfiguration_t get_default_configuration(void)
{
     ...
     */
    config.weight_position_x = FIXED_POINT_ONE;                                      /* 0.0 - disabled */
    config.weight_position_y = FIXED_POINT_ONE;                                      /* 0.0 - disabled */
    config.weight_heading    = FIXED_POINT_ONE;                        /* 1.0 */
    config.weight_velocity   = FIXED_POINT_ONE;                        /* 1.0 */
     ...
     */
    config.weight_steering_rate  = FIXED_POINT_ONE;                    /* 1.0 */
    ...
    fixed_point_t *linear_cost_vector)
{
    ...
    for (int ci = 0; ci < horizon_steps; ci++)
    {
        for (int cj = ci; cj < horizon_steps; cj++)
        {
            ...
            for (int a = 0; a < CONTROL_DIMENSION; a++)
            {
                for (int b = 0; b < CONTROL_DIMENSION; b++)
                {
                    fixed_point_t val = fixed_point_mul(FIXED_POINT_TWO, block[a][b]);
    ...
    fixed_point_t two_w_steer = fixed_point_mul(FIXED_POINT_TWO,
        current_configuration.weight_steering_effort);
    fixed_point_t two_w_vel = fixed_point_mul(FIXED_POINT_TWO,
    ...
    for (int ci = 0; ci < horizon_steps; ci++)
    {
        ...
        if (ci < horizon_steps - 1)
        {
            hessian_matrix[idx_s] = fixed_point_add(hessian_matrix[idx_s],
                fixed_point_mul((fixed_point_t)(4 * FIXED_POINT_ONE), w_sr));
            hessian_matrix[idx_v] = fixed_point_add(hessian_matrix[idx_v],
                fixed_point_mul((fixed_point_t)(4 * FIXED_POINT_ONE), w_vr));
        }
        else
        {
            hessian_matrix[idx_s] = fixed_point_add(hessian_matrix[idx_s],
                fixed_point_mul(FIXED_POINT_TWO, w_sr));
            hessian_matrix[idx_v] = fixed_point_add(hessian_matrix[idx_v],
                fixed_point_mul(FIXED_POINT_TWO, w_vr));
        }
        ...
        if (ci > 0)
        {
            ...
            fixed_point_t neg_2_sr = fixed_point_neg(fixed_point_mul(FIXED_POINT_TWO, w_sr));
            fixed_point_t neg_2_vr = fixed_point_neg(fixed_point_mul(FIXED_POINT_TWO, w_vr));
    ...
    for (int ci = 0; ci < horizon_steps; ci++)
    {
        for (int a = 0; a < CONTROL_DIMENSION; a++)
        {
            ...
            linear_cost_vector[ci * CONTROL_DIMENSION + a] =
                fixed_point_mul(FIXED_POINT_TWO, sum);
        }
    }
