...
/* Forward arc distance from s_from to s_to on the closed track. */
static double forward_track_delta(double s_from, double s_to)
{
    if (g_track_length_m <= 1e-6) {
        return s_to - s_from;
    }

    double delta = s_to - s_from;
    while (delta < 0.0) delta += g_track_length_m;
    while (delta >= g_track_length_m) delta -= g_track_length_m;
    return delta;
}
...
int main(void)
{
    ...
    double prev_progress_s = raceline[0].s;
    ...
    avg_progress_mps = (simulated_time > 1e-6) ? (progress_m / simulated_time) : 0.0;
    const double avg_lap_time = (completed_laps > 0) ? (lap_time_sum / completed_laps) : 0.0;
    ...
    if (getenv("MPC_TUNING_CSV")) {
         printf("CSV,%d,%d,%.4f,%.4f,%.4f,%.4f,%.2f,%.1f,%.1f,%d,%.1f,%.4f,%.4f,%.1f,%.4f,%.4f,%.4f,%d,%.4f,%.4f,%d\n",
               ...
               progress_m, avg_progress_mps, completed_laps, avg_lap_time,
