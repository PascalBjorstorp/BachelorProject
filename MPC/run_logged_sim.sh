...
cleanup() {
	...
	pkill -f 'ros2 launch mpc_f1_10th mpc_launch.py' 2>/dev/null || true
	pkill -f 'ros2 launch f1tenth_gym_ros gym_bridge_launch.py' 2>/dev/null || true
	pkill -f '/mpc_f1_10th/mpc_node' 2>/dev/null || true
...
echo "Launching MPC..."
ros2 launch mpc_f1_10th mpc_launch.py "trajectory_file:=${TRAJECTORY_FILE}" >"${MPC_LOG}" 2>&1 &
MPC_PID=$!

echo "Running for up to ${DURATION_SECONDS}s (stops early on first collision warning)..."
